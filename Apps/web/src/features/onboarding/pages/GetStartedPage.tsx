...
import { useMeSummaryQuery, useRegisterMutation } from '../../auth';
import {
  StepIndicator,
  StepParentAccount,
  StepChildProfile,
...
const API_ERROR_TRANSLATION_PATTERNS: Array<{ pattern: RegExp; key: string }> = [
...
const hasTranslationKey = (translations: TranslationMap, key: string): boolean => {
  return Object.prototype.hasOwnProperty.call(translations, key);
};

const mapApiMessageToTranslationKey = (message: string): string | null => {
...
const translateApiMessage = (message: string, translations: TranslationMap): string => {
  ...
  if (hasTranslationKey(translations, normalizedMessage)) {
    const translatedMessage = translations[normalizedMessage];
    return typeof translatedMessage === 'string' ? translatedMessage : normalizedMessage;
  }

  const mappedKey = mapApiMessageToTranslationKey(normalizedMessage);
  if (mappedKey) {
    const translatedMessage = translations[mappedKey];
    return typeof translatedMessage === 'string' ? translatedMessage : normalizedMessage;
  }
