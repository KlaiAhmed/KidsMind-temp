...
class SafetyAndRulesService:
    ...
    def patch_safety_and_rules(
        ...
        """Update child safety settings and/or parent PIN based on provided payload fields."""
        ...
        if payload.child_settings is not None and child_profile is not None:
            self.db.refresh(child_profile)
        self.db.refresh(current_user)

        return SafetyAndRulesPatchResponse(
            message="Safety and rules updated successfully",
            child_id=child_profile.id if child_profile else 0,
